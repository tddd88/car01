...
◆ 功能包: decision
  ...
  │  ├─ 功能: 建图时，通过手柄 B 键(2号按键)瞬间提取 /map 与 /base_link 关系并保存为 yaml（一次建图内，
              每次按下时均可以保存并覆盖之前的预备点配置）
  │  ├─ 订阅: /joy
  │  └─ 发布: 保存 yaml 配置文件
  │
  └─ 节点: race_state_machine
     ...
     ├─ 状态流转详见下方 4.1 详细说明
     ├─ 订阅: /blue_target, /move_base/status
...
**业务逻辑构想（非常稳健麦轮专属逻辑）：**
1. [起步导航]: 一键启动后读入 `waypoint_saver` 存下的预备点。发出导航请求，让底盘自主避障前往预备点。
2. [视觉抢占]: 去预备点途中，如果 `/blue_target` 持续发出有效的蓝色视野框，直接打断导航（取消 Action目标），进入 [视觉伺服] 状态。
3. **[极端情况补救（盲区搜索模式）]**: 如果因场地大幅度调整，车子顺利到达了终点预备坐标，但摄像头中完全没有发现蓝色。
    * 车体就地自转，强制正对终点挡板所处的方向（查阅建图时的角度）。
    * 由于咱们使用的是**麦克纳姆轮底盘**，此时可下发 `(V_x=0, V_y=0.2, V_omega=0)` 的横向平移指令。
    * 让车子面朝终点区，保持绝对平移进行巡场扫描（像螃蟹一样左右平移）。
    * 一旦在平移过程中，摄像头捕捉到了蓝色：立刻刹停平移，转入 [视觉伺服]。
4. [视觉伺服]: 对接视觉信息。X轴（直线）控制与蓝纸距离，Y轴（平移）保证蓝纸在画面中心，直至蓝纸占比超过预设阈值。下发速度 0 停车。
